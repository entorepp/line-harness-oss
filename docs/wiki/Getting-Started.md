...
チャット画面の送信仕様（日本語 LINE、台湾 LINE、WhatsApp 共通）:

- 通常の `送信` はサーバー側で30秒だけ保持し、画面左下に Gmail 風の小さなポップアップを表示します
- ポップアップの `元に戻す` は残り秒数が0になるまで利用でき、画像と本文など同時に登録した複数メッセージもまとめて取り消します
- 30秒後はチャット画面が即時に送信を確定します。画面を閉じた場合は毎分の Worker scheduled job がバックアップ送信します
- 時計ボタンの `予約送信` は通常送信とは別機能で、指定した日本時間まで保持され、予約一覧から変更・取消できます
- WhatsApp の `＋` から JPEG/PNG、PDF・Office文書・テキスト、MP4、対応音声ファイルを添付できます。画像は5MB、動画・音声は16MB、文書は現在のKV保存上限に合わせて25MBまでです
- LINE / WhatsApp の provider API に送信した後は相手側から取り消せないため、通常送信を取り消せるのは送信前の30秒だけです
